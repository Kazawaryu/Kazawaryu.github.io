...
The goal is to $$\min f(x)$$, subject to $$h(x)=0, g(x)\prec 0$$. For any feasible point $$x$$, i.e., such that $$h(x)<0, g(x)\preceq 0$$, define the set of *active Inequality constraints* by
