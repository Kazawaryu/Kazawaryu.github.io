...
## 3.2 Weak Pontryagin max principle in discrete-time

### 3.2.1 Optimal control problem (OCP)

Similar to normal optimal problem, where the cost is the combination of terminal + stage, and the constraints involve 1.) dynamical feasibility, 2.) initial condition, 3.) terminal condition, and 4.) input constraints. For instance, 

cost (terminal + stage):

$$\min_{x,u} \ell (x_T) + \sum_{t=0}^{T-1}\ell (t,x_t,u_t)$$

s.t. dynamical feasibility: $$x_{t+1}=f(t,x_t,u_t),\forall t\in\{0,1,\dots,T-1\}$$

initial condition: $$x_o = \bar{x}_0$$

terminal condition: $$x_T\in \mathcal{X}_T$$

input constraints: $$u_t\in\mathcal{U}, \forall t\in\{0,1,\dots,T-1\}$$
